...
def retrieval_eval(model, ref_model, data_loader, tokenizer, device, config):
    ...
    adv_save_dir = ""
    if export_adv:
        from baselines.export_adv import append_vlr_records, vlr_adv_dir, write_vlr_manifest

        adv_save_dir = vlr_adv_dir(save_dir)

    print('Forward')
    for step, (images, texts, texts_ids) in enumerate(data_loader):
...
def main(args, config):
    ...
    test_loader = DataLoader(test_dataset, batch_size=config['batch_size_test'], num_workers=0)
