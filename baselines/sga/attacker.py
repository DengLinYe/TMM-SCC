...
class SGAttacker():
    ...
    def attack(self, imgs, txts, txt2img, device='cpu', max_length=30, scales=None, **kwargs):
        ...
        ref_net = self.txt_attacker.ref_net
        ref_was_cuda = ref_net.device.type == "cuda"
        if ref_was_cuda:
            ref_net.cpu()
            torch.cuda.empty_cache()

        adv_imgs = self.img_attacker.txt_guided_attack(
            self.model, imgs, txt2img, device, scales=scales, txt_embeds=txt_supervisions
        )
...
class ImageAttacker():
    ...
    def _attack_views(self, adv_imgs, scales):
        views = [adv_imgs]
        if scales is None:
            return views
        ori_shape = (adv_imgs.shape[-2], adv_imgs.shape[-1])
        reverse_transform = transforms.Resize(
            ori_shape, interpolation=transforms.InterpolationMode.BICUBIC
        )
        for ratio in scales:
            ...
            scaled_imgs = adv_imgs + torch.randn_like(adv_imgs) * 0.05
            scaled_imgs = scale_transform(scaled_imgs)
            scaled_imgs = torch.clamp(scaled_imgs, 0.0, 1.0)
            views.append(reverse_transform(scaled_imgs))
        return views
    ...
    def txt_guided_attack(self, model, imgs, txt2img, device, scales=None, txt_embeds=None):
        ...
        for step in range(self.steps):
            ...
            grad_sum = torch.zeros_like(adv_imgs)
            views = self._attack_views(adv_imgs, scales)
            n_views = len(views)

            for view in views:
                model.zero_grad(set_to_none=True)
                norm_view = self.normalization(view) if self.normalization is not None else view
                view_out = model.inference_image(norm_view)
                loss = self.loss_func(view_out["image_feat"], txt_embeds, txt2img) / n_views
                loss.backward()
                if adv_imgs.grad is not None:
                    grad_sum.add_(adv_imgs.grad.detach())
                    adv_imgs.grad = None
                del view_out, loss, norm_view, view

            grad = grad_sum
            grad = grad / torch.mean(torch.abs(grad), dim=(1, 2, 3), keepdim=True).clamp(min=1e-8)
